...
void print_control_help()
{
    if (param::keyboard_control)
    {
        ...
        {
            if (param::sim_route_follow)
            {
                std::cout << "  Loopback default: starts in Parkour idle-hold; hold w/up to follow the parkour terrain route, release to stop.\n";
            }
            else
            {
        ...
        if (param::sim_loopback_interactive && param::sim_route_follow)
        {
            std::cout << "  w/up : route-follow only while held at " << param::sim_command_x << " m/s\n";
        }
        else
        {
