...
from scripts.antifall_harness import default_scenarios_for_task
...
def test_stage3_continues_the_flat_push_kick_ladder() -> None:
  stage3 = _env("Unitree-G1-AntiFall-Stage3")

  assert stage3.scene.terrain is not None
  assert stage3.scene.terrain.terrain_type == "plane"
  _assert_antifall_helpers_wired(stage3)
  assert stage3.events["push_robot"].func is mdp.push_by_setting_velocity_with_history
  assert "height_scan" not in stage3.observations["actor"].terms
  assert "height_scan" not in stage3.observations["critic"].terms
  assert stage3.events["push_robot"].interval_range_s == (2.75, 4.25)
  profile = stage3.events["push_robot"].params["velocity_range"]
  assert profile["x"] == (-0.9, 0.9)
  assert profile["pitch"] == (-0.8, 0.8)
  assert stage3.events["reset_base"].params["hard_reset_prob"] == 0.25


def test_stage4a_and_stage4b_progress_the_push_kick_difficulty_instead_of_slip_or_trip() -> None:
  ...
  assert stage4a.scene.terrain.terrain_type == "plane"
  assert stage4b.scene.terrain.terrain_type == "plane"
  assert stage4a.events["push_robot"].func is mdp.push_by_setting_velocity_with_history

  lateral_profile = stage4a.events["push_robot"].params["velocity_range"]
  assert lateral_profile["y"] == (-1.15, 1.15)
  assert lateral_profile["x"] == (-0.75, 0.75)
  assert stage4a.events["reset_base"].params["hard_reset_prob"] == 0.3

  assert stage4b.events["push_robot"].func is mdp.push_by_setting_velocity_with_history
  hardest_profile = stage4b.events["push_robot"].params["velocity_range"]
  assert hardest_profile["x"] == (-1.75, 1.75)
  assert hardest_profile["y"] == (-1.75, 1.75)
  assert hardest_profile["z"] == (-1.0, 1.0)
  assert hardest_profile["roll"] == (-1.25, 1.25)
  assert hardest_profile["pitch"] == (-1.25, 1.25)
  assert hardest_profile["yaw"] == (-1.5, 1.5)
  assert stage4b.events["reset_base"].params["hard_reset_prob"] == 0.35


def test_antifall_play_cfg_allocates_headroom_for_drag_contact_bursts() -> None:
  for task_id in (
    "Unitree-G1-AntiFall-Stage0",
    "Unitree-G1-AntiFall-Stage1",
    "Unitree-G1-AntiFall-Stage2",
    ...
    cfg = _env(task_id, play=True)
    assert cfg.sim.nconmax == 256


def test_benchmark_scenarios_follow_the_push_kick_reset_for_late_stages() -> None:
  stage3 = default_scenarios_for_task("Unitree-G1-AntiFall-Stage3")
  stage4a = default_scenarios_for_task("Unitree-G1-AntiFall-Stage4a")
  stage4b = default_scenarios_for_task("Unitree-G1-AntiFall-Stage4b")

  assert all("pushkick" in scenario.tags for scenario in stage3)
  assert all("rough" not in scenario.tags for scenario in stage3)

  assert all("pushkick" in scenario.tags for scenario in stage4a)
  assert all("slip" not in scenario.tags for scenario in stage4a)

  assert all("pushkick" in scenario.tags for scenario in stage4b)
  assert all("trip" not in scenario.tags for scenario in stage4b)
