...
class AMPPPO:
    ...
    def init_storage(
        ...
        """Create rollout storage using the current rsl_rl TensorDict API."""
        obs = TensorDict(
            {
                "actor": torch.zeros(num_envs, *actor_obs_shape, device=self.device),
                "critic": torch.zeros(num_envs, *critic_obs_shape, device=self.device),
            },
            batch_size=[num_envs],
            device=self.device,
        )
        if self.rnd:
            obs["rnd_state"] = torch.zeros(num_envs, self.rnd.num_states, device=self.device)
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            if hasattr(sample, "observations"):
                obs_batch = sample.observations["actor"]
                critic_obs_batch = sample.observations["critic"]
                ...
                old_mu_batch, old_sigma_batch = sample.old_distribution_params
                hid_states_batch = sample.hidden_states
                masks_batch = sample.masks
                rnd_state_batch = sample.observations.get("rnd_state", None)
            else:
                (
                    obs_batch,
                    critic_obs_batch,
