...
# -----------------------------------------------------------------------------
# Local AMP compatibility helpers copied from AMP_mjlab's rsl_rl fork.
# -----------------------------------------------------------------------------

import os as _amp_os
import pathlib as _amp_pathlib
from typing import Tuple as _AmpTuple

import git as _amp_git
import numpy as _amp_np

_AMP_EPS = _amp_np.finfo(float).eps * 4.0


class RunningMeanStd:
    def __init__(self, epsilon: float = 1e-4, shape: _AmpTuple[int, ...] = ()):  # noqa: UP006
        self.mean = _amp_np.zeros(shape, _amp_np.float64)
        self.var = _amp_np.ones(shape, _amp_np.float64)
        self.count = epsilon

    def update(self, arr: _amp_np.ndarray) -> None:
        batch_mean = _amp_np.mean(arr, axis=0)
        batch_var = _amp_np.var(arr, axis=0)
        batch_count = arr.shape[0]
        self.update_from_moments(batch_mean, batch_var, batch_count)

    def update_from_moments(self, batch_mean: _amp_np.ndarray, batch_var: _amp_np.ndarray, batch_count: int) -> None:
        delta = batch_mean - self.mean
        tot_count = self.count + batch_count
        new_mean = self.mean + delta * batch_count / tot_count
        m_a = self.var * self.count
        m_b = batch_var * batch_count
        m_2 = m_a + m_b + _amp_np.square(delta) * self.count * batch_count / (self.count + batch_count)
...
class Normalizer(RunningMeanStd):
    ...
    def normalize(self, input):
        return _amp_np.clip((input - self.mean) / _amp_np.sqrt(self.var + self.epsilon), -self.clip_obs, self.clip_obs)
...
def store_code_state(logdir, repositories) -> list:
    git_log_dir = _amp_os.path.join(logdir, "git")
    _amp_os.makedirs(git_log_dir, exist_ok=True)
    file_paths = []
    for repository_file_path in repositories:
        try:
            repo = _amp_git.Repo(repository_file_path, search_parent_directories=True)
            tree = repo.head.commit.tree
        except Exception:
            print(f"Could not find git repository in {repository_file_path}. Skipping.")
            continue
        repo_name = _amp_pathlib.Path(repo.working_dir).name
        diff_file_name = _amp_os.path.join(git_log_dir, f"{repo_name}.diff")
        if _amp_os.path.isfile(diff_file_name):
            continue
...
def string_to_callable(name: str) -> Callable:
    """AMP-fork compatibility alias for the newer resolve_callable helper."""
