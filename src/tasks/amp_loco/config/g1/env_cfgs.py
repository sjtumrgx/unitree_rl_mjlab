...
class AmpMotionFiles:
  """Resolved AMP motion files used by the locomotion/recovery task."""

  walk_run: tuple[str, ...]
  recovery: tuple[str, ...]


def _npz_files(path: str) -> tuple[str, ...]:
  return tuple(
...
def g1_amp_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  ...
  cfg.amp_motion_files = AmpMotionFiles(
    walk_run=_npz_files(_motion_dir),
    recovery=_npz_files(_recovery_dir),
  )
